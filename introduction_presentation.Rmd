...
* **Regression** tries to predict numeric (continuous) variable (*e.g.*, how many MPG do we expect this car to have?)  
* **Classification** tries to predict categorical (discrete) variable (*e.g.*, what type of species do we expect this iris to be?)  
...
-- *e.g.*, `cars_split <- initial_split(mtcars, prop = 0.75)`  

* `rsample::training` and `rsample::testing` then used to extract data  

-- e.g., `cars_training <- training(cars_split)` and `testing_data <- testing(cars_split)`  
...
Importantly, these steps require data to be performed - but we don't want to use information from the testing dataset (that would be "data leakage")

Therefore, we specify design matrices or blueprints (`recipes`) that specify the desired steps, rather than pre-processing data directly (especially useful when we have to do this many times, like with cross-validation)  

## Data pre-processing using `recipes`  

* Basic function for initiating recipe is `recipes::recipe`  

-- *e.g.*, `cars_recipe <- recipe(mpg ~. data = cars_training)`
...
* We estimate the parameters for the recipe using `prep` (*e.g.*, `cars_recipe_prepped <- prep(cars_recipe, training = cars_training`))  

* We apply the prepped recipe to the training data using `juice` (*e.g.*, `cars_training_juiced <- juice(cars_recipe_prepped)`)  

* We apply the prepped recipe to the testing data using `bake` (*e.g.*, `cars_testing_baked <- bake(cars_testing)`)  
...
cars_fit_lm <- cars_model_lm %>%
  fit(mpg ~ ., data = cars_training_juiced)
