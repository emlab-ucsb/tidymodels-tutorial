...
# Regression example using `mtcars`
...
cars_recipe_prepped <- cars_recipe %>%
...
cars_training_juiced <- juice(cars_recipe_prepped)

cars_training_juiced

# We use the bake function to apply the prepped recipe to the testing dataset
cars_testing_baked <- cars_recipe_prepped %>%
  bake(cars_testing) 
...
predict(cars_fit_ranger, cars_testing_baked)

# Add these ranger predictions to the testing dataset
cars_fit_ranger %>%
  predict(cars_testing_baked) %>%
  bind_cols(cars_testing_baked)
...
all_model_results <- all_models %>%
  ...
  mutate(model_fit = purrr::map(model,
                                ~fit(.x, mpg ~ ., data = cars_training_juiced)),
...
credit_split <- initial_split(GermanCredit, prop = 0.75, strata = "Class")
